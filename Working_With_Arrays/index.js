...
const checkDogs = function (dogsJulia, dogsKate) {
  const dogJulia_new = removeCats(dogsJulia);
  const bothDogs = dogJulia_new.concat(dogsKate);
...
const calcAverageHumanAge = (dogAges) => {
  const dog_adult = dogAges
    .map((dogAge) => {
      return dogAge <= 2 ? 2 * dogAge : 16 + dogAge * 4;
    })
    .filter((dogAge) => dogAge >= 18);
  return (
    dog_adult.reduce((init, cur) => {
      return init + cur;
    }, 0) / dog_adult.length
  );
};
...
let Data_1 = [5, 2, 4, 1, 15, 8, 3];
let Data_2 = [16, 6, 10, 5, 6, 1, 4];
console.log("calcAverageHumanAge");
console.log("Data 1");
console.log(calcAverageHumanAge(Data_1));
console.log("Data 2");
